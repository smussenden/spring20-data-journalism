...
# Filter the county pills data frame just for Maryland counties.  Then group by buyer state and year and add up pills to get a total for each state 
...
# Build the chart  
  ...
  labs(x="Year", y="Total pills", title="In Maryland, opioids rise through 2011 before falling", subtitle = "Total pills shipped to Maryland by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)
  
# The year-over-year-trend in Maryland and Baltimore are different.  In Baltimore, pills spiked sharpest in 2006, before falling. In Maryland, the rise was gradual through 2011, before falling.
...
# Get state-by-state-population-data
state_population_2012 <- state_population(key=key) %>%
  clean_names() %>%
  filter(year==2012)

# Filter county-by-year totals just for 2012, then group by buyer_state and add up total pills
state_pills_2012 <- arcos_county_pills_per_year %>%
  filter(year == 2012) %>%
  group_by(buyer_state) %>%
  summarize(dosage_unit = sum(dosage_unit)) 

# Join state_pills_per_year to county_by_year

state_pills_population_2012 <- state_population_2012 %>%
  inner_join(state_pills_2012) 

# Create a "pills per person" column and sort sheet on that column, from highest to lowest
...
# Print out the dataframe showing Kentucky is the highest
state_pills_population_2012
...
# Kentucky had the highest rate, at 72.1 pills per person
