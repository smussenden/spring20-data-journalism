...
Q: Build a bar chart that shows 

Q: Which state had the highest rate of 

```{r}
drugs <- drug_list(key=key)
state_population_2014 <- state_population(key=key) %>%
  clean_names() %>%
  filter(year==2014)
...
pills_pharmacy_maryland <- total_pharmacies_state(state="MD", key=key)

pharmacies_prince_georges <- total_pharmacies_county(county = "Prince Georges", state = "MD", key = "WaPo")
