...
medicare_opioids <- read_csv("data/medicare_opioids_dirty.csv") %>%
  # clean up the column names to standardize
  clean_names() %>%
  # remove the two totally blank rows  
  remove_empty_rows() %>%
  # remove the row with statewide totals
  filter(!str_detect(county, "Statewide")) %>%
  # separate the combined county and state column (county) into a separate column for county and state
  separate(county, into=c("county","state"), sep=",") %>%
  # trim the whitespace off of the state column 
  mutate(state = str_trim(state)) %>%
  # Remove the word County from the county column
  mutate(county = str_remove(county, "County")) %>%
  # trim the whitespace off of the county column %>%
  mutate(county = str_trim(county)) %>%
  # Make the county coulumn and state column all lowercase
  mutate(county = tolower(county),
         state = tolower(state)) %>%
  # Make a new column for state code
  mutate(state_code = str_sub(fips, start=1L, end=2L)) %>%
  # Make the NO VALUE PROVIDED into proper N/As
  ...
  # Convert all the rate columns to numeric values
