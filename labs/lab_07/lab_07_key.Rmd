...
First, lets pull in county and state.  The "variable" here is just a count of total population, but it doesn't really matter.  We just want the geometry, which we get by selecting gemoetry=TRUE.  
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Starting slow, pill shipments rapidly increase between 2006 and 2012 ", subtitle = "Pills per person shipped to each state, 2006 to 2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)

```

**Task**: Make a single INTERACTIVE map that is colored based on the number of pills per person in each county in 2009. Provide a short writeup on what you find.What trends do you see, beyond Appalachia? Note: the map will probably open in a web broswer.

```{r}

# Filter pills population table just for 2009.  
